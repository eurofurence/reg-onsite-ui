...
async def setup(
    ...
    global _access_token, _merchant_code, _job, _generation
    _access_token = token
    _merchant_code = merchant_code
    # New credentials may point at a different account/con — bump the
    # generation so any in-flight job from before this reset discards
    # its results instead of returning stale-account data.
    _job = None
    _generation += 1
    return {"status": "ok"}
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            for task in detail_tasks:
                if task.exception() is not None:
                    continue
                for key, quantity in task.result():

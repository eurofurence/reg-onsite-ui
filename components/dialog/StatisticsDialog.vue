<template>
    <Dialog v-model:visible="visible" modal :dismissableMask="true" :closeOnEscape="false" :maximizable="true"
        header="Statistics" :style="{ width: '80vw' }">
        ...
        <div class="flex flex-wrap gap-5 justify-content-center">
            <Chart type="pie" :data="getOverviewData(dataSource, 'status', configStatusItems)" :options="chartOptions"
                class="w-25rem" />
            <Chart type="pie" :data="getOverviewData(dataSource, 'sponsor', configSponsorItems)" :options="chartOptions"
                class="w-25rem" />
            <Chart type="pie" :data="getOverviewData(dataSource, 'conbook', configConbookItems)" :options="chartOptions"
                class="w-25rem" />
            <Chart type="pie" :data="getOverviewData(dataSource, 'roles', configRoleItems, true)" :options="chartOptions"
                class="w-25rem" />
        </div>
    </Dialog>
</template>
